...
MQTT_HELPER_STATIC void mqtt_state_set(enum mqtt_state new_state) {
    ...
    if (mqtt_state_get() == new_state) {
        LOG_DBG("Skipping transition to the same state (%s)", state_name_get(mqtt_state_get()));
        return;
    }
    ...
    LOG_DBG("State transition: %s --> %s", state_name_get(mqtt_state), state_name_get(new_state));
...
static int certificates_provision(void) {
    ...
    if (sizeof(ca_certificate) > 1) {
        ...
        if (err == -EEXIST) {
            LOG_DBG("CA certificate already exists, sec tag: %d", CONFIG_MQTT_HELPER_SEC_TAG);
        } else if (err < 0) {
    ...
    if (sizeof(private_key) > 1) {
        err = tls_credential_add(CONFIG_MQTT_HELPER_SEC_TAG,
                                 ...
                                 sizeof(private_key));
        if (err == -EEXIST) {
            LOG_DBG("Private key already exists, sec tag: %d", CONFIG_MQTT_HELPER_SEC_TAG);
        } else if (err < 0) {
    ...
    if (sizeof(device_certificate) > 1) {
        ...
        if (err == -EEXIST) {
            LOG_DBG("Public certificate already exists, sec tag: %d", CONFIG_MQTT_HELPER_SEC_TAG);
        } else if (err < 0) {
...
MQTT_HELPER_STATIC void mqtt_evt_handler(struct mqtt_client* const mqtt_client,
    ...
    if (current_cfg.cb.on_all_events != NULL) {
        if (!current_cfg.cb.on_all_events(mqtt_client, (const struct mqtt_evt* const)mqtt_evt)) {
    ...
    switch (mqtt_evt->type) {
        ...
        case MQTT_EVT_PUBACK:
            ...
            if (current_cfg.cb.on_puback) {
                current_cfg.cb.on_puback(mqtt_evt->param.puback.message_id, mqtt_evt->result);
            }
            break;
        case MQTT_EVT_SUBACK:
            ...
            if (current_cfg.cb.on_suback) {
                current_cfg.cb.on_suback(mqtt_evt->param.suback.message_id, mqtt_evt->result);
            }
            break;
...
static int broker_init(struct sockaddr_storage* broker,
    ...
    while (addr != NULL) {
        if (addr->ai_family == AF_INET6) {
            ...
            net_ipaddr_copy(&broker6->sin6_addr, &((struct sockaddr_in6*)addr->ai_addr)->sin6_addr);
            broker6->sin6_family = addr->ai_family;
            broker6->sin6_port   = htons(CONFIG_MQTT_HELPER_PORT);

            zsock_inet_ntop(addr->ai_family, &broker6->sin6_addr, addr_str, sizeof(addr_str));
            LOG_DBG("IPv6 Address found %s (%s)", addr_str, net_family2str(addr->ai_family));
            break;
        } else if (addr->ai_family == AF_INET) {
            struct sockaddr_in* broker4 = ((struct sockaddr_in*)broker);

            net_ipaddr_copy(&broker4->sin_addr, &((struct sockaddr_in*)addr->ai_addr)->sin_addr);
            broker4->sin_family = addr->ai_family;
            broker4->sin_port   = htons(CONFIG_MQTT_HELPER_PORT);

            zsock_inet_ntop(addr->ai_family, &broker4->sin_addr, addr_str, sizeof(addr_str));
            LOG_DBG("IPv4 Address found %s (%s)", addr_str, net_family2str(addr->ai_family));
            break;
        } else {
...
static int client_connect(struct mqtt_helper_conn_params* conn_params) {
    ...
    if (IS_ENABLED(CONFIG_MQTT_HELPER_SEND_TIMEOUT)) {
        ...
        if (err == -1) {
            ...
            LOG_DBG("Using send socket timeout of %d seconds", CONFIG_MQTT_HELPER_SEND_TIMEOUT_SEC);
        }
    }
...
MQTT_HELPER_STATIC void mqtt_helper_poll_loop(void) {
    ...
    while (true) {
        if (!mqtt_state_verify(MQTT_STATE_CONNECTING) && !mqtt_state_verify(MQTT_STATE_CONNECTED)) {
        ...
        ret = zsock_poll(fds, ARRAY_SIZE(fds), mqtt_keepalive_time_left(&mqtt_client));
        if (ret < 0) {
...
K_THREAD_DEFINE(mqtt_helper_thread,
                CONFIG_MQTT_HELPER_STACK_SIZE,
                mqtt_helper_run,
                false,
                NULL,
                NULL,
